...
public class MoradorUtil {
    ...
    public static void lerDadosCandidatos() {
    	...
    	for (int i = 0; i < qtd; i++) {
			...
			boolean unidadeJaCandidata = false;
			for(int j = 0; j < candidatos.size(); j++){
				if (candidatos.get(j).getNumeroUnidade() == unidade) {
					unidadeJaCandidata = true;
					break;
				}   
			}
			
			if (unidadeJaCandidata) {
				System.out.println("Já existe um candidato da unidade " + unidade + ". Não é permitido mais de um candidato por unidade.");
			} else {
